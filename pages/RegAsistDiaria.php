...
<div id="page-wrapper">
 <div class="row" align="center">
  <div class="col-lg-10"><div class="tile"><h2 class="tile-title" ><font color="#85C1E9"><center><b>Asistencias Diarias</b></font></h2>  </div></div>
  </div> <!-- /.row titulo --><br>
<?php
    //Listo los cursos
                 ...
                 if(!empty($Asist)) 
				   {
                    ...
                    if(!empty($detalleExiste)) 
					 {
 ?>
					  <div class="alert alert-dismissible alert-danger"><center><strong>La asistencia ya fue registrada con anterioridad</strong></center></div>
<?php
                     }
				    else 
					 {
                      ...
                      if($_POST['Situacion']!=2)
                       ...
                       if (guardarAsistenciaYDetalle($MiConexion,$_SESSION['EstudEle'],$anio,$_SESSION['InstanciaEle'],$_POST['Fecha'],$_POST['Situacion'],$_POST['Justificacion'])) 
						{
?>
                          <div class="bs-component"><div class="alert alert-dismissible alert-success"><strong><center>¡Asistencia guardada!</center></strong></div></div>
<?php
                          $Lugar=0;
...
<?php
                ...
                }// cierra else si todo esta completo
              }// cierra if Confirmar
?>
		 </div><!-- Cierra col carteles de errores-->
...
?>
           <option value="<?php echo $ListadoInstancias[$i]['ID']; ?>" <?php echo $selectedI; ?>  > <?php echo $ListadoInstancias[$i]['DENOMINACION']; ?></option>
<?php 
		  }  
?>
        </select></div>
		<div class="col-lg-2"><button type="submit" class="btn btn-primary" value="ElegirInstancia" name="ElegirInstancia"><box-icon  name="show-alt"  size="sm" color="white" animation="tada-hover"></box-icon> Ver</button></div>
	</div><!--Cierra row Instancia--><br>
<?php  
	if(!empty($_POST['ElegirInstancia']))
	  {
 ...
 ?>
    ...
    <div class="row"align="center">
     <div class="form-group">
      ...
      <div class="col-lg-2"> <input id="date" type="date" name="Fecha" value="<?php echo !empty($_POST['Fecha']) ? $_POST['Fecha'] : ''; ?>"></div>
	<div class="col-lg-1"></div>
	  <div class="col-lg-2"><label>Situación* </label></div>
      <div class="col-lg-4"> <select class="form-control" name="Situacion" id="Situacion">
...
?>
         <option value="<?php echo $ListadoSituaciones[$i]['ID']; ?>" <?php echo $selectedS; ?>  ><?php echo $ListadoSituaciones[$i]['DENOMINACION']; ?></option>
<?php 
		}  ?>
         </select></div>
		 </div><!--Cierra form group-->
		...
		Acá va la acción del botón SituacionNueva-->
         </div><!--Cierra row Fecha-Situacion--><br>
        <div class="row" >
		 <div class="col-lg-1"></div>
		 <div class="col-lg-10"><div class="form-group"><label>Justificación</label> <textarea class="form-control" rows="2" name="Justificacion"><?php echo !empty($_POST['Justificacion']) ? $_POST['Justificacion'] : ''; ?></textarea></div></div>
        </div><!--Cierra row Justificacion--><br>
        <div class="row" align="center">
		...
		</div><!--Cierra row aviso--><br>
        <div class="row" align="center">
		 <div class="col-lg-2"></div>
		 <div class="col-lg-4"><button type="submit" class="btn btn-primary" value="Confirmar" name="Confirmar" onClick="return confirm ('¿Desea guardar la asistencia?');"><box-icon  name="check-double"  size="sm" color="white" animation="tada"></box-icon> Confirmar</button></div>
		<div class="col-lg-4"><button type="submit" class="btn btn-danger" value="Cancelar" name="Cancelar" onclick="return confirm ('¿Desea retornar? - No se guardarán los datos.')"><box-icon  name="arrow-back"  size="sm" color="white" animation="tada-hover"></box-icon> Retornar</button>
       </div>
	   </div><!--Cierra row botones-->
					    </form>
