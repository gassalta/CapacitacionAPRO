...
function selectDocenteMail($Dni,$Mail){
	$consultaDocente=$consulta="Select * from docentes where DNI= '".$Dni."' and Mail= '".$Mail."'";
    $SqlCliente=mysqli_query(ConexionBD(),$consultaDocente);
	return $SqlCliente;
}
...
function UpdatePasword($codigo,$Dni){
   $cambio="Update docentes set Contrasenia='$codigo' where DNI='$Dni'";
   $SqlCambio=mysqli_query(ConexionBD(),$cambio);
}
